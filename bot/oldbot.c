...
char *host = "ncohafmuta.com";                     /* REPLACE THIS */
int   port = 5000;                            /* REPLACE THIS */

/* BOT_NAME and ROOT_ID must be all lowercase */
#define BOT_NAME      "spokes"                       /* REPLACE THIS */
#define CONNMSG       "\n spokes \n maple2\0"        /* REPLACE THIS */
#define ROOT_ID       "cygnus"                       /* REPLACE THIS */
#define DCONMSG       ".q\0"
...
#define MAIN_ROOM     "front_gate" /* the main room of the talker */
#define HOME_ROOM     "pond"       /* the bot room set in constants.h */
#define STORYDIR      "Stories"
#define DIRECTORY     "."
#define LOG_FILE      "log.log"

/* Should be set same as in constants.h */
#define NAME_LEN	21
#define MAX_USERS	40
...
/* Current directions - last %s is the bot's home room */
...
sendchat (fmt, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10)
char *fmt;
...
listen_to_people()
{
	...
	*/
	...
	u=get_user_num(playername);
	 ...
	 if (logging) { fputs(inbuf,log_fp); fputs("\n",log_fp); }
...
int handle_action(playername, lowername, command, volume)
  ...
  if (!strcasecmp(command, "help")) 
	...
	goto END;
      ...
      if (i < story->numkids)
        ...
        sprintf(buffer+strlen(buffer), "%c\0", 'a' + story->numkids);
  ...
  if (strlen(command) == 0) {
    if (story->numlines == 0) {           /* Author is done */
      ...
      /* If not, start a new para! */
        ...
        sprintf(buffer, "%s%c\0", story->paragraphid, 'a' + story->numkids);
...
save_para(story, playername, lowername)
struct storystate *story;
char *playername, *lowername;
{
  char buffer[MAXSTRLEN];
  FILE *fp;
  int i;
...
int mode;
{
  ...
  if (mode==1) abreakup+=15;
	...
	sprintf(lsdir,"ls %s/%s",DIRECTORY,STORYDIR);
  if ((fp = popen(lsdir, "r")) == NULL) {
   ...
   if (mode==1) {
    ...
    while (fgets(titlebuf, MAXSTRLEN, fp) != NULL) {
...
int acceptprefix;
{
  ...
  if (strlen(prefix) == 0)
    return NULL;
  storymatch = 0;
  sprintf(tmp,"/bin/ls %s/%s",DIRECTORY,STORYDIR);
  if ((fp = popen(tmp, "r")) == NULL) {
    sendchat(".say I'm sorry, but I can't get to my story directory!\n");
    sendchat(".say Cygnus should know about this-- this shouldn't happen.\n");
  }
  else {
    while (fgets(inbuf, MAXSTRLEN, fp) != NULL && !exactmatch) {
...
struct storystate *story;
{
  ...
  if (c != '\0') {
    sprintf(story->paragraphid+strlen(story->paragraphid), "%c\0", c);
    get_paragraph(story);
    read_paragraph(story);
  }
}
...
handle_page2(pageline,playername,lowername)
char *pageline;
char *playername, *lowername;
{
