...
/* */
	  ...
	  if ((query=get_value(request,"search"))==NULL) {
	   query="\0";
	   /* web_error(user,HEADER,BAD_REQUEST); goto FREE; */
	   }
...
if (!strcmp(woption,"prefs")) {
 ...
 for (z=0;z<MAX_GRAVOKES;++z) {
        if (!isrevoke(t_ustr.revokes[z])) continue;
...
if (!(tfp=fopen(filename,"r"))) {
	write_log(ERRLOG,YESTIME,"WWW: Couldn't open file \"%s\" in get_mime_type! %s",filename,get_error());
